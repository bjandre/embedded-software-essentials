...
/**
 */
void profile_memmove(log_item_t *item, uint32_t num_bytes, uint8_t *destination, uint8_t *source);

/**

 */
void profile_memset(log_item_t *item, uint32_t num_bytes, uint8_t *destination, uint8_t source);
...
#undef NUM_BYTES
    ...
    for (size_t nb = 0; nb < num_bytes; nb++) {
        ...
        profile_memset(item, bytes[nb], destination, cp_value);
...
void profile_memmov_sys(uint32_t num_bytes, log_item_t* item,
...
void profile_memmov_cpu(uint32_t num_bytes, log_item_t* item, uint8_t* destination,
		uint8_t* source) {
	profiling_timer_data_t start_time, end_time;
	uint32_t total_clocks;
	uint32_t average_clocks;
	...
	UpdateLogItem(item, PROFILING_VALUE, sizeof(uint32_t), &average_clocks);
	log_item(item);
	UpdateLogItem(item, PROFILING_END, sizeof(name_memmove_cpu),
			&name_memmove_cpu);
	log_item(item);
}
...
void profile_memmove(log_item_t *item, uint32_t num_bytes, uint8_t *destination, uint8_t *source)
{
	profile_memmov_sys(num_bytes, item, destination, source);
	profile_memmov_cpu(num_bytes, item, destination, source);
	profile_memmov_dma(num_bytes, item, destination, source);

}

void profile_memset_system(uint8_t source, uint32_t num_bytes,
		log_item_t* item, uint8_t* destination) {
    profiling_timer_data_t start_time, end_time;
...
void profile_memset_cpu(uint8_t source, uint32_t num_bytes,
		log_item_t* item, uint8_t* destination) {
	...
	UpdateLogItem(item, PROFILING_START, sizeof(name_memset_cpu),
			&name_memset_cpu);
	log_item(item);
	for (uint8_t i = 0; i < num_trials; i++) {
		get_timer(&start_time);
	...
	UpdateLogItem(item, PROFILING_VALUE, sizeof(uint32_t), &average_clocks);
	log_item(item);
	UpdateLogItem(item, PROFILING_END, sizeof(name_memset_cpu),
			&name_memset_cpu);
	log_item(item);
}

void profile_memset_dma(uint8_t source,
		uint32_t num_bytes, log_item_t* item, uint8_t* destination) {
...
void profile_memset(log_item_t *item, uint32_t num_bytes, uint8_t *destination, uint8_t source)
{
	profile_memset_system(source, num_bytes, item, destination);
	profile_memset_cpu(source, num_bytes, item, destination);
	profile_memset_dma(source, num_bytes, item, destination);
}
